...
class MHModel:
    ...
    def _predict(self, p, t, eff, disc, spill):
        ...
        if spill.shape[0]: mult = mult*(1+(th[:,None]*spill).sum(0))
        return base*mult
...
def run(keep_partial=True, log_scale=True):
...
def _dashboard(panels,fits,wp,shape,log_scale):
    ...
    tab=["PARAMETERS  (sale = assumed-shape: P*=$15 rho=0.4 kappa=3)\n"]
    for g in TRAIN:
...
if __name__=="__main__":
    ...
    a=ap.parse_args()
    run(keep_partial=not a.no_partial, log_scale=not a.linear)
